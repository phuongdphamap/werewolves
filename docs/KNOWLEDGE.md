...
**The chronicle too** (B-79). Entries are rendered to a string at the moment they happen
and never re-rendered, which sounds fragile and is not: the language chip lives on the deck
screen, unreachable once the first night starts, so a game cannot change language midway
and a chronicle cannot end up half in each. `lang-test.js` pins that placement — moving the
control into the Roster would silently break the guarantee, so the test fails if a second
`setPref('lang')` appears anywhere.

A resumed save from a game played in the other language keeps its own words, which is what
a record should do.
...
37. **Render-at-write is sound only while the input cannot change.** The chronicle stores
    finished strings, so translating the writers can never retranslate history — normally a
    reason to store structured data and render late. It is fine here for one reason only:
    the language chooser is unreachable once a game starts. That is a property of the
...
40. **A loose read and a strict read of the same field will disagree eventually.** Every
    house-rule accessor asks `G.x == null`, which is true for an absent key; the chip asked
    `G.x === val`, which is not. So two rules that never reached `blank()` worked perfectly
